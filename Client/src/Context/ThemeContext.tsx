...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({
  ...
  const isFundManagerSession = user?.role === "fundManager";

  // Extract selectedThemeId from user.selectedTheme object
  const userSelectedThemeId = user?.selectedTheme;

  // Enhanced loading states — start ready so login never blocks on theme fetch
  ...
  const {
    ...
    { enabled: isFundManagerSession },
    userSelectedThemeId ?? undefined
  );
  const shouldFetchThemes =
    isFundManagerSession &&
    isAuthenticated &&
    !!user &&
  ...
  useEffect(() => {
    if (currentUserId.current !== userId) {
      ...
      if (!userId || !isFundManagerSession) {
        setIsInitializing(false);
        setIsThemeReady(true);
        setCurrentTheme(defaultTheme);
        setSelectedThemeId(null);
  ...
  }, [userId, dashboardType, isFundManagerSession]);

  // MAIN THEME INITIALIZATION EFFECT - Improved priority handling
  useEffect(() => {
    if (!isFundManagerSession || dashboardType !== "fundManager" || !userId || !user) {
      return;
    }
    ...
    const initializeThemeFromData = () => {
      try {
        ...
        // Priority 4: Never block UI while theme API is still loading
        setCurrentTheme(loadThemeFromStorage(userId) || defaultTheme);
        setIsInitializing(false);
        setIsThemeReady(true);
      } catch (error) {
  ...
  // Only block UI when actively applying a theme — not while fetching in background
  const isLoading =
    isFundManagerSession &&
    (applyThemeMutation.isPending || isApplyingTheme);
