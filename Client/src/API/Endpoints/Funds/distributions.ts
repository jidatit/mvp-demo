...
import {
  mockGetDistributions,
  mockCreateDistribution,
  mockUpdateDistributionStatus,
...
export const useGetDistributions = ({
  ...
  return useQuery<PaginatedDistributions, Error>({
    ...
    queryFn: async () => {
      ...
      return mockGetDistributions(page, limit, fundId);
    },
    staleTime: 0,
    refetchOnMount: true,
...
export const useCreateDistribution = (userId: string) => {
  ...
  return useMutation<Distribution, Error, CreateDistributionPayload>({
    mutationFn: async (payload) => {
      return mockCreateDistribution(payload);
    },
    onSuccess: () => {
...
export const useUpdateDistributionStatus = (userId: string) => {
  ...
  return useMutation<Distribution, Error, UpdateDistributionStatusPayload>({
    mutationFn: async ({ id, status }) => {
      return mockUpdateDistributionStatus(id, status);
    },
    onSuccess: () => {
...
export const useUpdateDistribution = () => {
  ...
  >({
    mutationFn: async ({ id, payload }) => {
      return mockUpdateDistribution(id, payload);
    },
    onSuccess: () => {
