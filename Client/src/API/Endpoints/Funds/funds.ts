...
export const useUpdateFund = () => {
  ...
  return useMutation({
    mutationFn: async (params: UpdateFundPayload) => {
      ...
      return mockUpdateFund({ id: params.id, data: params.data });
    },
    onSuccess: () => {
      toast.success("Fund updated successfully!");
...
export const useGetInvestorFunds = () => {
  return useQuery<InvestorFundSummary[]>({
    ...
    queryFn: async () => {
      const response = await mockGetInvestorFunds();
      if (!response.success) {
        throw new Error(response.message || "Failed to fetch investor funds");
      }
      return response.data;
    },
