...
export const useGetNotifications = ({
  ...
  return useQuery<PaginatedNotifications, Error>({
    ...
    queryFn: async () => {
      ...
      return mockGetNotifications(userId, params);
    },
    staleTime: 0,
    refetchOnMount: true,
...
export const useMarkNotificationAsRead = () => {
  ...
  return useMutation<void, Error, { userId: string; notificationId: string }>({
    mutationFn: async ({ userId, notificationId }) => {
      await mockMarkNotificationRead(userId, notificationId);
    },
    onSuccess: (_, { userId }) => {
...
export const useMarkAllNotificationsAsRead = () => {
  ...
  return useMutation<void, Error, { userId: string }>({
    mutationFn: async ({ userId }) => {
      await mockMarkAllNotificationsRead(userId);
    },
    onSuccess: (_, { userId }) => {
...
export const useDeleteNotification = () => {
  ...
  return useMutation<void, Error, { userId: string; notificationId: string }>({
    mutationFn: async ({ userId, notificationId }) => {
      await mockDeleteNotification(userId, notificationId);
    },
    onSuccess: (_, { userId }) => {
